...
TEST_CASE("Evaluator: While pattern evaluator") {
    ...
    SECTION("Synonym WildCards Pair") {
        ...
        Table *table1 = WhilePatternEvaluator::evaluate(patternClause1, pkbManager);
    ...
    SECTION("Identifier WildCards Pair") {
        ...
        Table *table2 = WhilePatternEvaluator::evaluate(patternClause2, pkbManager);
    ...
    SECTION("Semantically & Syntactically Invalid") {
        ...
        REQUIRE_THROWS(WhilePatternEvaluator::evaluate(patternClause2, pkbManager));
        ...
        REQUIRE_THROWS(WhilePatternEvaluator::evaluate(patternClause3, pkbManager));

        PatternClause patternClause4(whileSyn, variableSyn, vector<PatternVariable>({patternFP}));
        REQUIRE_THROWS(WhilePatternEvaluator::evaluate(patternClause4, pkbManager));
