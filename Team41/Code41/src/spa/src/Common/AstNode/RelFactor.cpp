...
vector<TNode *> RelFactor::verify(TNodeType type, vector<TNode *> &children) {
  switch (type) {
	...
	case TNodeType::div:
	case TNodeType::mod:if (children.size() != 2) throw runtime_error("invalid number of children");
	default:break;
  }
  return children;
}
...
Token *RelFactor::verify(TNodeType type, Token *val) {
  switch (type) {
	...
	case TNodeType::constValue:if (val == nullptr) throw runtime_error("token val should not be null");
	default:break;
  }
  return val;
}
